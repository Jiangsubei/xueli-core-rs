...
impl TokenCounter {
    ...
    pub fn new_cl100k() -> Result<Self, String> {
        let bpe = tiktoken_rs::cl100k_base().map_err(|e| format!("加载 tiktoken BPE 失败: {}", e))?;
        Ok(Self { bpe })
    }

    /// 使用 o200k_base 编码创建计数器
    pub fn new_o200k() -> Result<Self, String> {
        let bpe = tiktoken_rs::o200k_base().map_err(|e| format!("加载 tiktoken BPE 失败: {}", e))?;
        Ok(Self { bpe })
    }
    ...
    pub fn count_messages(
        &self,
        messages: &[crate::traits::ai_client::ChatMessage],
    ) -> usize {
        messages
            .iter()
